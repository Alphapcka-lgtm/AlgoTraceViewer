...
export function SwapOutput(props: SwapOutputProps) {
    ...
    const labels = useMemo(() => createStepLabels(3 * props.steps.length + 3), [props.steps.length]);
        ...
        <div className="algorithm-panel">
            ...
            >
                ...
                {Array.from({length: props.values.length}, (_, i) => (
                    <React.Fragment key={`index-label-${i}`}>
                        <text
                ...
                <g>
                    ...
                    />
                    {kGraph.coordinates
                        .filter(c => (c.i + 1) % kLabelDistance === 0)
                        .map(c => (
                            <text
                                key={`k-label-${c.i}`}
                                x={c.x}
